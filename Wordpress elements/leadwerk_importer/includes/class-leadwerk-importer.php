<?php
/**
 * Main importer for Atelier Francis pages, media and translations.
 *
...
class Leadwerk_Importer {
	...
	protected function get_canonical_shell_html( $page_config ) {
		...
		if ( defined( 'LEADWERK_IMPORTER_PATH' ) ) {
			$candidates[] = trailingslashit( LEADWERK_IMPORTER_PATH ) . 'source_assets/' . $file_name;
		}
		if ( function_exists( 'leadwerk_theme_resolve_source_shell_path' ) ) {
			$resolved = leadwerk_theme_resolve_source_shell_path( $file_name );
			if ( '' !== $resolved ) {
				$candidates[] = $resolved;
			}
		}
	...
	protected function perform_preflight_step( $job_state ) {
		...
		$this->record_preflight_section_layout_warnings();
	...
	protected function record_preflight_source_asset_warnings() {
		...
		$logo_found = false;
		foreach ( array( 'Logo.webp', 'Logo.png' ) as $logo_name ) {
			if ( is_file( $fotos . DIRECTORY_SEPARATOR . 'Logo' . DIRECTORY_SEPARATOR . $logo_name ) ) {
				$logo_found = true;
				break;
			}
		}
		if ( ! $logo_found ) {
			Leadwerk_Logger::record_result(
				'warning',
				'Fotos/Logo/Logo.webp fehlt — Header/Footer-Logos im statischen HTML koennen nicht importiert werden.',
				'preflight'
			);
		}

		foreach ( array( 'styles.css', 'script.js' ) as $asset ) {
			if ( ! is_file( $root . DIRECTORY_SEPARATOR . $asset ) ) {
				Leadwerk_Logger::record_result(
					'warning',
					'Quell-Asset fehlt: ' . $asset,
					'preflight'
				);
			}
		}
		...
		if ( is_dir( $fotos ) ) {
			$iterator = new RecursiveIteratorIterator( new RecursiveDirectoryIterator( $fotos, RecursiveDirectoryIterator::SKIP_DOTS ) );
			foreach ( $iterator as $file ) {
				if ( $file->isFile() && preg_match( '/[^\x00-\x7F]/', $file->getPathname() ) ) {
					Leadwerk_Logger::record_result(
						'warning',
						'Umlaut-Pfad unter Fotos/ erkannt — Medienimport pruefen: ' . $file->getPathname(),
						'preflight'
					);
					break;
				}
	...
	/**
	 * Warn when HTML section count does not match schema layout count.
	 *
	 * @return void
	 */
	protected function record_preflight_section_layout_warnings() {
		if ( ! class_exists( 'Leadwerk_Content_Schema' ) || ! class_exists( 'DOMDocument' ) ) {
			return;
		}
		...
		foreach ( (array) ( $this->manifest['pages'] ?? array() ) as $page_config ) {
			...
			libxml_clear_errors();
			$xpath    = new DOMXPath( $dom );
			$sections = $xpath->query( '//body/main/section' );
			if ( ! ( $sections instanceof DOMNodeList ) || 0 === $sections->length ) {
				$sections = $xpath->query( '//body/section' );
			}
			$section_count = ( $sections instanceof DOMNodeList ) ? $sections->length : 0;
			$layout_count  = count( (array) $group['layouts'] );

			if ( $section_count !== $layout_count ) {
				Leadwerk_Logger::record_result(
					'warning',
					sprintf(
						'Section/Layout-Mismatch %s (%s): %d HTML-Sektionen vs %d Schema-Layouts',
						(string) ( $page_config['source_key'] ?? $source_file ),
						$source_file,
						$section_count,
						$layout_count
					),
	...
	protected function perform_page_upsert_step( $job_state, $batch_size ) {
		...
		while ( $cursor < $total && $processed < $batch_size ) {
			...
			$en_result = $this->upsert_page( $page_config, 'en' );
			...
			if ( ! $this->dry_run && ! empty( $de_result['post_id'] ) && ! empty( $en_result['post_id'] ) && class_exists( 'Leadwerk_Translation_API' ) ) {
			...
			if ( ! empty( $de_result['post_id'] ) && ! empty( $en_result['post_id'] ) ) {
	...
	protected function fill_options() {
		...
		$option_map = array(
			'header_logo' => 'Fotos/Logo/Logo.webp',
			'footer_logo' => 'Fotos/Logo/Logo.webp',
		);
		...
		if ( ! $this->leadwerk_get_option( 'company_address' ) ) {
			$this->leadwerk_update_option( 'company_address', "Kronenstraße 22\n76275 Ettlingen" );
		}

		if ( ! $this->leadwerk_get_option( 'company_phone' ) ) {
			$this->leadwerk_update_option( 'company_phone', '0721 84 42 83' );
		}

		if ( ! $this->leadwerk_get_option( 'company_email' ) ) {
			$this->leadwerk_update_option( 'company_email', 'info@atelierfrancis.de' );
		}

		if ( ! $this->leadwerk_get_option( 'google_maps_url' ) ) {
			$this->leadwerk_update_option( 'google_maps_url', 'https://www.google.com/maps/search/?api=1&query=Kronenstra%C3%9Fe+22+76275+Ettlingen' );
		}
		...
		if ( ! $this->leadwerk_get_option( 'theme_strings_de' ) || $this->theme_strings_need_francis_migration( $this->leadwerk_get_option( 'theme_strings_de' ) ) ) {
		...
		if ( ! $this->leadwerk_get_option( 'theme_strings_en' ) || $this->theme_strings_need_francis_migration( $this->leadwerk_get_option( 'theme_strings_en' ) ) ) {
	 ...
	 */
	/**
	 * Whether stored theme strings still contain ACM legacy copy.
	 *
	 * @param mixed $raw Stored option value.
	 * @return bool
	 */
	protected function theme_strings_need_francis_migration( $raw ) {
		if ( empty( $raw ) ) {
			return true;
		}
		...
		foreach ( array( 'footer_tagline', 'footer_copyright', 'header_logo_alt', 'footer_logo_alt' ) as $key ) {
			...
			if ( '' !== $value && preg_match( '/ACM|AIR CHARTER|Business Aviation|Rheinmünster|Charter, Management/i', $value ) ) {
				return true;
			}
		}
	...
	protected function set_site_icon() {
		...
		$attachment_id = $this->resolve_attachment( 'Fotos/Logo/Favicon.webp' );
		if ( ! $attachment_id ) {
			$attachment_id = $this->resolve_attachment( 'Fotos/Logo/Favicon.png' );
		}
		if ( $attachment_id ) {
