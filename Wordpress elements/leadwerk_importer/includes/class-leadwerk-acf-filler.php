...
require_once __DIR__ . '/trait-leadwerk-francis-filler.php';

class Leadwerk_ACF_Filler {
	use Leadwerk_Francis_Filler;
	...
	protected function get_selector_fallbacks( $field_name ) {
		$francis = $this->get_francis_selector_fallbacks( $field_name );
		if ( ! empty( $francis ) ) {
			return $francis;
		}
	...
	protected function parse_layout_value( $layout_key, $layout_schema, $section_node, $lang ) {
		...
		if ( 0 === strpos( $template, 'francis_' ) ) {
			$parsed = $this->parse_francis_layout( $template, $section_node );
			if ( is_array( $parsed ) ) {
				$parsed['acf_fc_layout'] = $layout_key;
				return $parsed;
			}
			return $this->empty_layout_value( $layout_key, $layout_schema );
		}
	...
	protected function parse_link_target( $href ) {
		...
		if ( '' === $raw && '' !== $fragment ) {
			return array(
				'page_key' => 'francis-index-v1',
				'url'      => $fragment,
			);
		}

		$page_key = $this->map_href_to_source_key( $raw );
		return array(
			'page_key' => $page_key,
			'url'      => '' !== $fragment ? $fragment : ( $this->is_internal_html_link( $raw ) ? '' : trim( (string) $href ) ),
		);
	}
	...
	protected function map_href_to_source_key( $href ) {
		...
		$map = $this->get_francis_href_map();
		if ( isset( $map[ $href ] ) ) {
			return $map[ $href ];
		}

		// Legacy ACM map fallback (unused after migration).
		$legacy = array(
		...
		return isset( $legacy[ $href ] ) ? $legacy[ $href ] : '';
	}
